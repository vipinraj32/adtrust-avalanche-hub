    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100 dark:from-slate-900 dark:via-purple-900/40 dark:to-blue-900/40">
      <div className="relative w-full max-w-md p-8 rounded-2xl shadow-2xl bg-white/30 dark:bg-slate-900/60 backdrop-blur-lg border border-white/20">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white mb-1 tracking-tight">Influencer Login</h2>
          <p className="text-sm text-slate-600 dark:text-slate-300">Access your influencer dashboard</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Username</label>
            <input name="username" type="text" placeholder="Username" value={form.username} onChange={handleChange} className="w-full p-3 rounded-lg border border-slate-300 dark:border-slate-700 bg-white/60 dark:bg-slate-800/60 focus:outline-none focus:ring-2 focus:ring-primary" required />
          </div>
          <div>
        ...
        <div className="text-center mt-6">
          ...
          <Button type="button" variant="outline" className="w-full mt-2" onClick={() => navigate('/influencer/signup')}>
